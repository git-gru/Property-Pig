...
?>
 <link rel="stylesheet" href="https://www.propertypig.co.uk/assets/css/style_property_pig.css">
  <style>
        #mytip{
            position:absolute;
            z-index:9999;
                ...
                <thead>
                                    ...
                                    foreach ($Customerlist_array as $val ) {
                                         echo '<tr id="customerlist_tr'.$val['user_id'].'">
												<td >'; ?>
                                    <a href="../user.php?id=<?php echo $val['user_id'];  ?>"><?php echo $val['Name'];  ?></a>
...
//        $("#targetDIV").html('<div class="panel-body"><address><strong>Name</strong><br><p>'+item.Name+'</p></address> <br/><address><strong>Phone number</strong><br><p>'+item.phoneNumber+'</p></address><br/><address><strong>Email id</strong><br><a href="mailto:#">'+item.email+'</a></address> <address><strong>User name</strong><br><p>'+item.userName+'</p></address><address> <br/><strong>Address</strong><br>'+item.Address+'</address> <address><strong>Zipcode</strong><br> <p>'+item.zipCode+'</p></address></div>');
    ...
    function questionpopup(item,status)
    {
		...
		var content ="";
		for(var i=0; i<unique_property.length; i++)
		{
			content += '<div class="row" ><b>Property :'+unique_property[i]+'</b></div>';
			content += '<table style="margin-bottom: 20px"><tr><td><b>Question</b></td><td style="padding-left: 20px"><b>Answer</b></td></tr>'
			$.each( question, function( key, value ) {
				console.log(value);
				if(unique_property[i]==value.property_id)
				{
					content += '<tr><td>'+value.question+'</td><td style="padding-left: 20px">'+value.answer+'</td></tr>'
					//content += '<div class="row" style="margin-left: 20px"><p>Question :'+value.question+'</p> <p>Answer&nbsp;&nbsp;&nbsp;: '+value.answer+'</p> </div>'
