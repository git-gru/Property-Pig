...
if ($query->rowCount() > 0) 
{
        ...
        if(!empty($_SESSION['qn']))
        {
           ...
           foreach ($_SESSION['qn'] as $value) 
           {
                ...
                $answer=$value['answer'];
                $type=$value['type'];
                $query3  = $dbh3->prepare("insert into pig_user_answer (`question_id`,`answer`,`question_type`,`user_id`,`property_id`) values(:key,:item,:typeid,:user_id,:propertyId)");

                $query3->bindParam(":key",$question_id,PDO::PARAM_INT);
                $query3->bindParam(":item",$answer,PDO::PARAM_STR); 
                $query3->bindParam(":typeid",$type,PDO::PARAM_INT); 
                $query3->bindParam(":user_id",$id,PDO::PARAM_INT); 
