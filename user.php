...
if($_SESSION['userid'] == '') {
    echo("<script>location.href = '" . $base_url . "admin/index.php';</script>");
    die();
}
...
if (!isset($_REQUEST['id'])) {
    ...
    $status = false;
    $row = '';
    $query = $dbh->prepare("select * from pig_user where user_id=:id");
...
<style>
    ...
    .halfsectionoregister:nth-child(2)::after{display:none;}
</style>
<!-----body section---->
                    ...
                    <div class="ndiv files-of-registration" page="register">
                        <form action="javascript:void(0);" id="form_register"   name="form_register"  method="POST" onsubmit="return update_profile();">
                            <div class="ndiv filelds-holder">
                                <input class="register-fields" disabled placeholder="Name" id="txtname" value="<?php echo @$rows['Name']; ?>" name="txtname">
                                <input class="register-fields" disabled type="number" min="0" placeholder="Phone" value="<?php echo @$rows['phoneNumber']; ?>" id="txtphone" name="txtphone">
                                <input class="register-fields" disabled type="email" placeholder="Email Id"  value="<?php echo @$rows['email']; ?>" id="txtemail" name="txtemail">
                                <input class="register-fields" disabled placeholder="User Name"  value="<?php echo @$rows['userName']; ?>" id="txtusername" name="txtusername">
                                <input class="register-fields" disabled placeholder="Address Line 1" value="<?php echo @$rows['Address']; ?>"  id="txtaddress" name="txtaddress">
                                <input class="register-fields" disabled placeholder="Address Line 2" value="<?php echo @$rows['Address1']; ?>" id="txtaddress1" name="txtaddress1">
                                <input class="register-fields" disabled placeholder="Zip Code"  value="<?php echo @$rows['zipCode']; ?>"  id="txtzip" name="txtzip">
