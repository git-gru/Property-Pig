...
?>  <style>.property-querys-holder{padding-top: 55px;}
.property-querys-holder {
   background-size: 11%;
}
</style>
                	...
                	<div class="ndiv property-detail-section">
                    	<table class="property-detail-table">
                            ...
                            <?php
                                ...
                                $pid =$_REQUEST['id'];
                                $query = $dbh->prepare("select * from pig_user_answer_rel where user_id=:user_id and property_id=:pid order by property_id asc");
...
<!--                                                         <td>
                                                    ...
                                                    ?>
                                                        ...
                                                        <?php
                                                            ...
                                                            $dbh3 = $obj3->dbh;
                                                            $query3 = $dbh3->prepare("select * from pig_property_status where  property_id=:property_id and user_id=:user_id and status='1'");
                    ...
                    <div class="ndiv property-querys-holder">
                    	<div class="ndiv property-querys-center">
                    		<div class="ndiv property-querys-in">
                                    ...
                                    <?php
                                    //echo "select * from pig_user_answer where user_id='" . $_SESSION['user_id'] . "' and property_id='" . $_REQUEST['id'] . "' ORDER BY id ASC";
                                    $querys = $dbh3->prepare("select * from pig_user_answer where user_id=:user_id and property_id=:property_id and answer!='' ORDER BY id ASC");
                                    $querys->bindParam(":property_id",$_REQUEST['id']);
                                    $querys->bindParam(":user_id",$_SESSION['user_id']);
                                    $querys->execute();
                                    ...
                                    if ($query2->rowCount() > 0) {
                                        ...
                                        while($rows_questions = $query2->fetch(PDO::FETCH_ASSOC))
                                        {
                                            ...
                                            ?>
                                    <div id="hide<?php echo $k; ?>" style="display: <?php if($k!=0){echo "none;";} ?>" >
    ...
    display: table;}
                                                           ...
                                                           <div class="ndiv yes-no-but-holder">
                                                               ...
                                                               <div class="ndiv ans-show-but"> 
                                                                   <?php
                                                                        ...
                                                                        $dbh0 = $obj0->dbh;
                                                                        $query0 = $dbh0->prepare("select * from pig_user_answer where user_id=:user_id and property_id=:property_id ORDER BY id ASC LIMIT $k,1");

                                                                        $query0->bindParam(":property_id",$_REQUEST['id']);
                                                                        $query0->bindParam(":user_id",$_SESSION['user_id']);
                                ...
                                if($my_counts_questions2>$m){ ?>
                                    ...
                                    <div class="ndiv yes-no-but-holder" style="height: 20px;width: initial;margin-bottom: 20px;">
                                        ...
                                        {
                                        ?>
                                        <button class="answer-choise-yes-no" onclick="go_back('<?php echo $k; ?>')" style="color:#fff;font-weight: bold;padding: 5px 10px;background: #8173c4;">Back</button>
                                        <?php }
                                        ...
                                        <button class="answer-choise-yes-no" style="color:#fff;font-weight: bold;padding: 5px 10px;background: #8173c4;" onclick="hide_me('<?php echo $k; ?>')">Next</button>
                                    </div>
                                <?php } else {?>   
                                <style>.property-query-questions{ min-height: 163px;}</style>
                                    <div class="ndiv yes-no-but-holder" style="height: 20px;width: initial;margin-bottom: 20px;">
...
$('.close-it').click(function(){
     ...
     $.ajax({
            type: "POST",
            data: "reason=" + reason + "&property_id=" + property_id + "&status=" + status + "&uid=" + uid,
            ...
            success: function (result) {  
                ...
                $('.yello-menu').trigger('click'); 
                $('.popup-header').html('NOTIFICATION');
                $('.popup-message').html('Thank you for submitting!');
                return false;
